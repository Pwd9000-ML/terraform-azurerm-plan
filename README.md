...
The terraform apply action will download and apply the artifact created by the plan action using the same `tf_key` and will either start a deployment or a destroy action based on the `plan_type`.  
